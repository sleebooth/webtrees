...
if ($to_delete) {
	foreach ($to_delete as $path) {
		...
		if (WT_File::delete(WT_DATA_DIR . $path)) {
			if ($is_dir) {
				WT_FlashMessages::addMessage(WT_I18N::translate('The folder %s was deleted.', WT_Filter::escapeHtml($path)));
			} else {
				WT_FlashMessages::addMessage(WT_I18N::translate('The file %s was deleted.', WT_Filter::escapeHtml($path)));
			}
		} else {
			if ($is_dir) {
				WT_FlashMessages::addMessage(WT_I18N::translate('The folder %s could not be deleted.', WT_Filter::escapeHtml($path)));
			} else {
				WT_FlashMessages::addMessage(WT_I18N::translate('The file %s could not be deleted.', WT_Filter::escapeHtml($path)));
			}
		}
	}
	...
	<div id="cleanup">
		...
		foreach ($entries as $entry) {
			if (in_array($entry, $do_not_delete)) {
				...
				echo '<input type="checkbox" id="', $uuid, '" name="to_delete[]" value="', WT_Filter::escapeHtml($entry), '">';
				echo '</label></li>';
			}
		}
		$dir->close();
